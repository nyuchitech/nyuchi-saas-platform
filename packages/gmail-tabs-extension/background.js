// Background Service Worker for Gmail Tabs Extension

// Handle extension installation and updates
chrome.runtime.onInstalled.addListener((details) => {
    console.log('Gmail Tabs Extension: Installation/Update detected', details.reason);
...
async function initializeExtension() {
    try {
        ...
        const result = await chrome.storage.local.get(['gmailTabs']);
        if (!result.gmailTabs) {
            const defaultTabs = [
            ...
            console.log('Gmail Tabs: Default tabs initialized');
        }
        ...
        if (!aiResult.aiSettings) {
            ...
            console.log('Gmail Tabs: Default AI settings initialized');
        }
        ...
        console.error('Gmail Tabs: Error initializing extension:', error);
    }
}
...
async function handleExtensionUpdate(previousVersion) {
    try {
        console.log(`Gmail Tabs: Updated from version ${previousVersion}`);
        ...
        console.error('Gmail Tabs: Error handling update:', error);
    }
}
    ...
    try {
        ...
        if (version[0] < 2) {
            ...
            if (!result.aiSettings) {
                ...
                await chrome.storage.local.set({ aiSettings: defaultAISettings });
                console.log('Gmail Tabs: Migrated to version 2.0.0 - added AI settings');
            }
        }
        
    } catch (error) {
        console.error('Gmail Tabs: Error migrating settings:', error);
    }
}
...
function showWelcomeNotification() {
    chrome.notifications.create({
        type: 'basic',
        iconUrl: 'icon48.png',
        title: 'Gmail Tabs with AI - Welcome!',
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    console.log('Gmail Tabs: Received message:', request);
    ...
    switch (request.action) {
        ...
        default:
            console.log('Gmail Tabs: Unknown action:', request.action);
    }
});
...
async function getTabAnalytics() {
    try {
        ...
        console.error('Gmail Tabs: Error getting analytics:', error);
        return {};
    }
}
...
async function clearTabAnalytics() {
    try {
        ...
        console.error('Gmail Tabs: Error clearing analytics:', error);
...
async function exportSettings() {
    try {
        ...
        console.error('Gmail Tabs: Error exporting settings:', error);
        return {
            success: false,
            error: error.message
        };
